...
class Wav2vec_U(BaseFairseqModel):
    def calc_gradient_penalty(self, real_data, fake_data):
        ...
        if not isinstance(real_data, RealData):
            real_data = RealData(features=real_data, padding_mask=torch.zeros(real_data.shape[:-1], dtype=torch.bool, device=real_data.device))
        ...
        if self.cfg.probabilistic_grad_penalty_slicing:
            ...
            b_size = min(real_data.features.size(0), fake_data.size(0))
            t_size = min(real_data.features.size(1), fake_data.size(1))
            real_feats = get_slice(real_data.features, 0, b_size)
            real_feats = get_slice(real_feats, 1, t_size)
            fake_data = get_slice(fake_data, 0, b_size)
            fake_data = get_slice(fake_data, 1, t_size)

        else:
            real_feats = real_data.slice_like(fake_data)
            fake_data = fake_data[: real_feats.size(0), : real_feats.size(1)]

        alpha = torch.rand(real_feats.size(0), 1, 1, device=real_feats.device)
        alpha = alpha.expand(real_feats.size())

        interpolates = alpha * real_feats + ((1 - alpha) * fake_data)
        ...
        gradients = autograd.grad(
            ...
            inputs=interpolates,
            grad_outputs=torch.ones(disc_interpolates.size(), device=real_feats.device),
            create_graph=True,
            retain_graph=True,
            only_inputs=True,
        )[0]
